...
model = LM_NN_Classifier(input_dim, hidden_dim, output_dim, max_iters=50)
model.train(train_data_pca, train_labels)
preds = model.predict(test_data_pca)

accuracy = np.mean(preds == test_labels)
print(f"Classification Accuracy: {accuracy * 100:.2f}%")

params_filename = "lm_nn_params.npy"
np.save(params_filename, model.params)
